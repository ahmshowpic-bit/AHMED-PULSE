...
// طبقات الخلفية: بنعزلها في Component مستقل (memo) عشان ما يتعاد رسمها مع كل
// نبضة تقدم، وبنوقف حركتها وهي مقفولة أو الأغنية واقفة.
const Atmosphere = React.memo(({ cover, active }: { cover: string; active: boolean }) => (
  <>
...
// علامات المدار: بتتعاد رسمها بس لما عدد العلامات المنورة يتغير (كل كام ثانية)،
// مش مع كل نبضة. النبض محصور في آخر 8 علامات منورة (ذيل الشهاب) بدل 72 حركة.
const OrbitTicks = React.memo(({ litCount, animate }: { litCount: number; animate: boolean }) => (
  <>
    {Array.from({ length: TICKS }, (_, i) => {
      const major = i % 6 === 0;
      const lit = i < litCount;
      const tail = animate && lit && i >= litCount - 8;
      return (
        <line
          key={i}
          x1={100}
  ...
  return (
    <>
      ...
      >
        ...
        <Atmosphere cover={cover} active={isActive} />
          ...
          <div className="min-h-full flex flex-col items-center justify-center gap-4 md:gap-6 px-4 py-3">
            ...
            >
              ...
              <svg
                ...
                aria-hidden="true"
              >
                <OrbitTicks litCount={litCount} animate={isActive} />
                <g
                  style={{
