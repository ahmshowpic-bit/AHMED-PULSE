...
const App: React.FC = () => {
  ...
  const likePost = (id: string) => {
    ...
    runTransaction(postRef, (likes) => (likes || 0) + 1).catch(err => console.warn(err.message));
  };
    ...
    <div className="relative h-[100dvh] w-screen max-w-[100vw] flex overflow-hidden">
      <style>
        {`
          ...
          .glass-panel {
            background: rgba(var(--theme-base-black), 0.4);
            backdrop-filter: blur(24px);
            -webkit-backdrop-filter: blur(24px);
            border: 1px solid rgba(var(--theme-base-white), 0.08);
      ...
      <aside className="hidden md:flex w-[280px] bg-black/40 backdrop-blur-2xl border-l border-white/5 z-50 flex-col p-8 transition-all shadow-[-20px_0_40px_rgba(0,0,0,0.5)] relative">
        ...
        <nav className="flex-1 space-y-2">
          ...
          {customPages.map(page => (
            <SidebarBtn key={page.id} active={activeTab === page.id} onClick={() => setActiveTab(page.id)} icon={<MoreHorizontal />} label={page.title} />
          ))}
        </nav>

        <div className="mt-auto flex justify-center pt-4">
          <button onClick={() => isAdmin ? setShowAdminModal(true) : handleLogin()} className="text-white/20 hover:text-cyan-400 transition-colors duration-300">
            <Shield size={24} />
          </button>
        </div>
      </aside>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <section className={`${activeTab === 'home' ? 'block' : 'hidden'} animate-fade-in`}>
              ...
              {heroSong && (
                <div className="animate-fade-in-up flex flex-col items-center gap-8 relative z-20" style={{ animationDelay: '0.2s' }}>
                  <div className="relative group cursor-pointer" onClick={() => playSong(heroSong, [heroSong])}>
                    ...
                    <img src={heroSong.image} alt={heroSong.name} className="w-48 h-48 md:w-64 md:h-64 rounded-full object-cover border-4 border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.6)] relative z-10 group-hover:scale-105 transition-transform duration-700 ease-out" />
                    <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 scale-75 group-hover:scale-100">
                      <div className="bg-white/20 backdrop-blur-md border border-white/30 rounded-full p-6 shadow-2xl">
                        <Play fill="currentColor" className="w-10 h-10 text-white ml-2 drop-shadow-md" />
                      </div>
                    </div>
                  </div>

                  <div className="text-center">
                    <h2 className="text-4xl md:text-6xl font-black text-white mb-4 drop-shadow-[0_5px_15px_rgba(0,0,0,1)] tracking-tight">{heroSong.name}</h2>
                    <div className="inline-flex items-center gap-3 bg-black/40 backdrop-blur-xl px-6 py-2 rounded-full border border-white/10 shadow-2xl">
                      ...
                      <span className="text-cyan-400 text-sm md:text-base font-black uppercase tracking-[0.2em]">{heroSong.folder}</span>
                    </div>
                  </div>

                  <button onClick={() => playSong(heroSong, [heroSong])} className="relative overflow-hidden bg-white text-black px-12 py-5 rounded-full font-black text-lg hover:scale-105 active:scale-95 transition-all duration-300 shadow-[0_20px_40px_rgba(255,255,255,0.1)] group flex items-center gap-3 mt-4 hover:shadow-cyan-500/20">
            ...
            <div className="mb-20">
              <div className="flex items-center justify-between mb-8 px-4 md:px-2">
                <h3 className="text-3xl font-black flex items-center gap-3 drop-shadow-md"><Sparkles className="text-yellow-400 animate-pulse" /> مختارات صوتية</h3>
              ...
              <div className="carousel-container flex gap-6 overflow-x-auto no-scrollbar pb-8 px-4 md:px-2 snap-x snap-mandatory">
                {featuredSongs.map(song => (
                  <div key={song.id} onClick={() => playSong(song, featuredSongs)} className="carousel-item flex-shrink-0 w-64 md:w-80 bg-black/40 backdrop-blur-xl border border-white/10 rounded-[2.5rem] overflow-hidden group cursor-pointer hover:border-cyan-500/50 hover:shadow-[0_15px_30px_rgba(6,182,212,0.15)] transition-all duration-500 snap-center">
            ...
            <div className="mb-20">
              <div className="flex items-center justify-between mb-8 px-4 md:px-2">
                <h3 className="text-3xl font-black flex items-center gap-3 drop-shadow-md"><Zap className="text-purple-400" /> نبض المجتمع</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 md:px-2">
                {latestDiaries.map(post => (
                  <div key={post.id} className="bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem] hover:bg-white/5 hover:border-purple-500/30 transition-all duration-500 flex flex-col gap-6 group hover:shadow-[0_10px_30px_rgba(168,85,247,0.1)] relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-500 to-purple-600 flex items-center justify-center font-black text-lg text-white shadow-lg">{post.name[0]}</div>
                      ...
                      <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center group-hover:bg-red-500/10 transition-colors"><Heart size={18} className="text-white/20 group-hover:text-red-400 transition-colors" /></div>
                    </div>
          ...
          <section className={`${activeTab === 'music' ? 'block' : 'hidden'} animate-fade-in-up`}>
            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-16 mt-12 gap-6">
              <h2 className="text-4xl md:text-5xl font-black flex items-center gap-4 drop-shadow-md">
                <div className="w-16 h-16 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.2)]"><MusicIcon size={32} /></div>
                المكتبة الصوتية
              </h2>
            </div>
            ...
            {currentFolder ? (
              <div className="space-y-6 animate-fade-in">
                <button onClick={() => setCurrentFolder(null)} className="inline-flex items-center gap-3 text-white/50 hover:text-white mb-8 transition-all bg-white/5 hover:bg-white/10 px-6 py-3 rounded-full border border-white/10 hover:border-cyan-500/30 group">
                  <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" /> <span className="font-bold">رجوع للمجلدات</span>
                </button>
                ...
                <div className="grid gap-4">
                  {folders[currentFolder]?.map((song) => (
                    <div key={song.id} onClick={() => playSong(song, folders[currentFolder])} className={`flex items-center gap-6 p-4 rounded-3xl cursor-pointer transition-all duration-300 border backdrop-blur-md group ${currentSong?.id === song.id ? 'bg-cyan-500/10 border-cyan-500/40 shadow-[0_10px_30px_rgba(6,182,212,0.15)] scale-[1.02]' : 'bg-black/40 border-white/5 hover:bg-white/5 hover:border-white/20'}`}>
                      ...
                      {currentSong?.id === song.id ? (
                        ...
                        <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white/20 group-hover:text-cyan-400 group-hover:border-cyan-500/30 transition-all opacity-50 group-hover:opacity-100"><Play size={20} className="ml-1" /></div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 animate-fade-in">
                {Object.keys(folders).map(folder => (
                  <div key={folder} onClick={() => setCurrentFolder(folder)} className="bg-black/40 backdrop-blur-xl border border-white/10 p-8 rounded-[3rem] flex flex-col items-center text-center cursor-pointer hover:-translate-y-2 hover:border-cyan-500/50 transition-all duration-300 group relative overflow-hidden shadow-xl hover:shadow-[0_20px_40px_rgba(6,182,212,0.15)]">
                    <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                    <div className="absolute top-0 right-0 p-6 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-4 group-hover:translate-x-0"><ChevronRight size={24} className="text-cyan-400" /></div>
                    <div className="w-24 h-24 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-[2rem] flex items-center justify-center text-cyan-400 mb-8 group-hover:scale-110 group-hover:bg-cyan-500 group-hover:text-black transition-all duration-500 shadow-2xl relative z-10"><FolderOpen size={48} className="group-hover:animate-bounce" /></div>
            ...
            {hasMoreSongs && !currentFolder && (
              <button onClick={loadMoreSongs} disabled={loadingMore} className="mt-10 w-full py-5 rounded-[2rem] bg-white/5 border border-white/10 text-white/60 font-black hover:bg-white/10 hover:text-white transition-all disabled:opacity-40">
          ...
          <section className={`${activeTab === 'diaries' ? 'block' : 'hidden'}`}>
              ...
              <div className="relative z-10">
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div className="flex items-center gap-3 px-4 py-2 rounded-2xl bg-white/5 border border-white/5">
                    <Sparkles size={20} className="text-yellow-400" /> <span className="text-xs text-white/40">شاركنا لحظاتك المميزة.</span>
                  </div>
                </div>
                ...
                <button onClick={postDiaryEntry} className="w-full md:w-auto px-12 py-5 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-full font-black text-lg shadow-2xl shadow-cyan-500/20 hover:scale-[1.05] active:scale-95 transition-all flex items-center justify-center gap-3 float-left">
            ...
            <div className="grid grid-cols-1 gap-8">
              ...
              {diaries.map(post => (
                <div key={post.id} className={`rounded-[2.5rem] border border-white/10 overflow-hidden shadow-xl transition-all hover:border-white/20 ${post.verified ? 'bg-gradient-to-br from-cyan-900/20 to-black/40 border-cyan-500/30 ring-1 ring-cyan-500/10' : 'bg-white/5'}`}>
                  <div className="flex items-center gap-4 p-6 bg-black/30 backdrop-blur-md">
                    ...
                    {isAdmin && (
                      <button onClick={() => remove(ref(db, `diaries/${post.id}`)).catch(e => console.error(e))} className="w-10 h-10 rounded-full bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"><Trash2 size={18} /></button>
                    )}
                  </div>
                  ...
                  <div className="p-4 bg-black/10 border-t border-white/5 px-8 flex justify-between items-center">
                    <button onClick={() => likePost(post.id)} className={`flex items-center gap-2 transition-all font-black py-2 px-4 rounded-full ${post.likes > 0 ? 'bg-red-500/10 text-red-500' : 'text-white/20 hover:text-white hover:bg-white/5'}`}>
              ...
              {hasMoreDiaries && (
                <button onClick={loadMoreDiaries} disabled={loadingMore} className="w-full py-5 rounded-[2rem] bg-white/5 border border-white/10 text-white/60 font-black hover:bg-white/10 hover:text-white transition-all disabled:opacity-40">
          ...
          <section className={`${activeTab === 'contact' ? 'block' : 'hidden'} animate-fade-in-up`}>
            <div className="max-w-3xl mx-auto bg-black/50 backdrop-blur-2xl border border-white/10 p-8 md:p-16 rounded-[4rem] text-center shadow-[0_20px_60px_rgba(0,0,0,0.8)] mt-12 relative overflow-hidden">
              ...
              <div className="w-28 h-28 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 rounded-[2.5rem] flex items-center justify-center text-cyan-400 mx-auto mb-10 shadow-[0_0_30px_rgba(6,182,212,0.2)] relative z-10"><Mail size={56} /></div>
              ...
              <div className="space-y-8 text-right relative z-10">
                <div className="group">
                  ...
                  <input value={contactName} onChange={e => setContactName(e.target.value)} placeholder="اكتب اسمك هنا" className="w-full bg-black/40 border border-white/10 p-6 rounded-3xl text-white text-center text-xl focus:ring-2 ring-cyan-500/50 outline-none transition-all group-hover:border-white/20 shadow-inner" />
                </div>
                <div className="group">
                  <label className="text-sm font-black text-cyan-400 block mb-3 mr-4 uppercase tracking-wider">محتوى الرسالة</label>
                  <textarea value={contactMsg} onChange={e => setContactMsg(e.target.value)} placeholder="بماذا تود أن تخبرني؟" rows={6} className="w-full bg-black/40 border border-white/10 p-6 rounded-[2.5rem] text-white resize-none text-center text-xl focus:ring-2 ring-cyan-500/50 outline-none transition-all group-hover:border-white/20 shadow-inner" />
                </div>
                <button onClick={sendMessage} className="w-full py-6 bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 rounded-[2.5rem] font-black text-2xl shadow-[0_15px_30px_rgba(6,182,212,0.3)] hover:shadow-[0_20px_40px_rgba(6,182,212,0.5)] hover:-translate-y-1 active:translate-y-1 transition-all mt-8 flex items-center justify-center gap-4 group">
              ...
              <div className="mt-20 opacity-[0.02] hover:opacity-100 transition-opacity duration-1000 relative z-20">
                <button onClick={() => isAdmin ? setShowAdminModal(true) : handleLogin()} className="p-4 rounded-full border border-dashed border-white/20 hover:border-cyan-400 hover:text-cyan-400 hover:shadow-[0_0_20px_cyan] hover:bg-cyan-900/40 transition-all" aria-label="Secure Login">
      ...
      >
        <div className="absolute -top-3 left-8 right-8 h-3 cursor-pointer group z-20 py-1" onClick={(e) => onSeek(e)}>
          <div className="w-full h-1 group-hover:h-2 bg-white/10 rounded-full overflow-hidden relative transition-all duration-300">
            <div className="absolute top-0 right-0 h-full bg-gradient-to-l from-cyan-400 to-purple-500 shadow-[0_0_10px_#00f2ff] transition-all duration-200" style={{ width: `${progress}%` }} />
          </div>
        </div>
        <div className="flex items-center gap-2 md:gap-5 flex-1 z-10 min-w-0">
          <div className="relative group flex-shrink-0">
            ...
            <div className={`w-10 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-xl md:rounded-2xl bg-cover bg-center border border-white/10 shadow-lg transition-transform duration-700 relative z-10 ${isPlaying ? 'scale-110 rotate-3 shadow-[0_10px_20px_rgba(0,0,0,0.5)]' : ''}`} style={{ backgroundImage: `url(${currentSong?.image || "https://images.unsplash.com/photo-1614850523459-c2f4c699c52e?q=80&w=400"})` }} />
          </div>
        ...
        <div className="flex items-center justify-center gap-2 md:gap-6 lg:gap-8 z-10 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
          ...
          <button onClick={(e) => togglePlay(e)} className="w-10 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-full bg-white text-black flex items-center justify-center shadow-[0_10px_20px_rgba(255,255,255,0.2)] hover:scale-110 hover:shadow-[0_15px_30px_rgba(255,255,255,0.3)] active:scale-95 transition-all duration-300" aria-label="تشغيل/ايقاف">
        ...
        <div className="flex items-center justify-end flex-shrink-0 z-10 gap-2 md:gap-4 ml-2 md:ml-0">
          <div className="hidden lg:flex items-center gap-3 bg-white/5 p-2 rounded-full border border-white/5 pr-4" onClick={e => e.stopPropagation()}>
            ...
            <input type="range" min="0" max="1" step="0.01" value={volume} onChange={e => setVolume(parseFloat(e.target.value))} className="w-20 h-1 accent-cyan-400 bg-white/10 rounded-full cursor-pointer" />
          </div>
          <button className="text-white/40 hover:text-white hover:scale-110 transition-all p-1.5 md:p-2 bg-white/5 rounded-full backdrop-blur-md border border-white/5"><Maximize2 size={16} className="md:w-5 md:h-5" /></button>
        </div>
      </div>

      {/* --- Full Screen Expanded Player --- */}
      <div className={`fixed inset-0 z-[200] flex flex-col transition-transformers duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${isPlayerExpanded ? 'translate-y-0 opacity-100' : 'translate-y-[100%] opacity-0 pointer-events-none'}`}>
        ...
        <div className="p-8 flex justify-between items-center relative z-10">
          <button onClick={() => setIsPlayerExpanded(false)} className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-lg flex items-center justify-center text-white hover:bg-white/20 border border-white/10 hover:scale-110 active:scale-95 transition-all shadow-xl" aria-label="إغلاق"><ChevronDown size={32} /></button>
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-6 outline-none relative z-10 pb-16 md:pb-20 overflow-y-auto">
          <div className="relative group mb-6 md:mb-10">
            ...
            <div className={`rounded-[2rem] md:rounded-[3rem] bg-cover bg-center shadow-[0_30px_60px_rgba(0,0,0,0.8)] border border-white/10 transition-transform duration-1000 relative z-10 ${isPlaying ? 'scale-100' : 'scale-95 grayscale-[20%]'}`} style={{ backgroundImage: `url(${currentSong?.image || "https://images.unsplash.com/photo-1614850523459-c2f4c699c52e?q=80&w=600"})`, width: 'clamp(240px, 60vw, 420px)', height: 'clamp(240px, 60vw, 420px)', }} />
          </div>
          <h2 className="font-black text-white mb-3 tracking-tighter drop-shadow-2xl text-center px-4 leading-tight" style={{ fontSize: 'clamp(1.5rem, 5vw, 4rem)' }}>{currentSong?.name || "اختر أغنية للبدء"}</h2>
          ...
          <div className="w-full max-w-xl md:max-w-3xl mb-8 md:mb-14 px-4" onClick={onSeek}>
            <div className="h-2 md:h-3 bg-white/10 rounded-full overflow-hidden relative cursor-pointer group shadow-inner">
              <div className="absolute top-0 right-0 h-full bg-gradient-to-l from-cyan-400 via-blue-500 to-purple-600 transition-all duration-200" style={{ width: `${progress}%` }} />
            </div>
          </div>
          <div className="flex items-center justify-center gap-6 md:gap-10 lg:gap-16">
            ...
            <button onClick={(e) => togglePlay(e)} className="rounded-full bg-white text-black flex items-center justify-center shadow-[0_20px_50px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 transition-all duration-300" style={{ width: 'clamp(80px, 15vw, 112px)', height: 'clamp(80px, 15vw, 112px)' }}>
      ...
      {showAdminModal && (
        <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-2xl flex items-center justify-center p-4">
          <div className="w-full max-w-5xl h-[90vh] bg-black border border-white/10 rounded-[3rem] flex flex-col overflow-hidden shadow-2xl ring-1 ring-white/10">
                  ...
                  <div className="space-y-6">
                    ...
                    {messages.map(m => (
                      <div key={m.id} className="bg-white/5 border border-white/10 p-6 rounded-[2rem] flex items-start gap-6 hover:bg-white/10 transition-all group">
                        <div className="w-16 h-16 rounded-[1.5rem] bg-cyan-600/20 flex items-center justify-center text-cyan-400 font-black text-2xl uppercase">{m.name[0]}</div>
                        <div className="flex-1">
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-[3rem] mb-12 space-y-6 shadow-xl ring-1 ring-white/5">
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <select value={selectedFolder} onChange={e => setSelectedFolder(e.target.value)} className="w-full bg-black/60 border border-white/10 p-4 rounded-2xl text-lg font-bold">
                        ...
                        {selectedFolder === 'new' && (
                          <div className="space-y-2">
                            ...
                            <input value={newFolderName} onChange={e => setNewFolderName(e.target.value)} placeholder="أدخل اسماً للمجلد" className="w-full bg-black/60 border border-white/10 p-4 rounded-2xl text-lg" />
                          </div>
                        )}
                      </div>
                      <div className="grid md:grid-cols-2 gap-6">
                      ...
                      {songs.map(s => (
                        <div key={s.id} className="flex items-center gap-6 p-4 bg-white/5 rounded-[1.5rem] border border-white/5 hover:border-white/10 transition-all group">
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => update(ref(db, 'settings'), { defaultSongId: s.id }).catch(e => console.error(e))} className={`w-12 h-12 rounded-full transition-all flex items-center justify-center ${settings.defaultSongId === s.id ? 'bg-yellow-500 text-black' : 'bg-white/5 text-white/20'}`}>
                  ...
                  <div className="space-y-10">
                    ...
                    <div className="space-y-8 bg-white/5 p-10 rounded-[3rem] border border-white/10 shadow-2xl">
                      <div className="space-y-3">
                        ...
                        <input value={settings.welcome} onChange={e => setSettings({ ...settings, welcome: e.target.value })} className="w-full bg-black/60 border border-white/10 p-5 rounded-2xl text-xl font-black text-center" />
                      </div>
                      <div className="flex items-center justify-between p-6 bg-cyan-500/5 rounded-[2rem] border border-cyan-500/10">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input type="checkbox" className="sr-only peer" checked={settings.showVisitorCount} onChange={e => setSettings({ ...settings, showVisitorCount: e.target.checked })} />
                      ...
                      <div className="p-10 bg-purple-500/5 rounded-[3rem] border border-purple-500/20 space-y-8 relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" className="sr-only peer" checked={settings.heroMode} onChange={e => setSettings({ ...settings, heroMode: e.target.checked })} />
                        ...
                        <div className="space-y-3">
                          ...
                          <input value={settings.heroImg} onChange={e => setSettings({ ...settings, heroImg: e.target.value })} placeholder="ضع الرابط هنا" className="w-full bg-black/60 border border-white/10 p-5 rounded-2xl text-lg font-mono" />
                        </div>
                        <div className="grid md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            <label className="text-xs font-black text-white/40 mr-2">نوع الوسائط</label>
                            <select value={settings.heroType} onChange={e => setSettings({ ...settings, heroType: e.target.value as any })} className="w-full bg-black/60 border border-white/10 p-4 rounded-2xl font-bold">
                          ...
                          <div className="space-y-2">
                            ...
                            <select value={settings.bgFit} onChange={e => setSettings({ ...settings, bgFit: e.target.value as any })} className="w-full bg-black/60 border border-white/10 p-4 rounded-2xl font-bold">
                      ...
                      <button onClick={() => { update(ref(db, 'settings'), settings).then(() => alert("تم تحديث كافة الإعدادات بنجاح")).catch(e => alert("فشل الحفظ: " + e.message)); }} className="w-full py-6 bg-gradient-to-r from-cyan-600 to-purple-600 rounded-[2rem] font-black text-2xl shadow-2xl shadow-cyan-600/30 hover:scale-[1.01] active:scale-95 transition-all">حفظ التعديلات</button>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-white/5 p-10 rounded-[3rem] border border-white/10 space-y-6 shadow-2xl">
                      ...
                      <button onClick={() => {
                        ...
                        } else { alert("يرجى إكمال الحقول الأساسية"); }
                      }} className="w-full py-6 bg-purple-600 rounded-full font-black text-xl shadow-2xl shadow-purple-600/20 hover:scale-[1.02] transition-all">نشر الصفحة الجديدة</button>
                    </div>
                    <div className="grid gap-4">
...
const MobNavBtn: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all duration-500 flex-1 relative ${active ? 'text-cyan-400 -translate-y-4' : 'text-white/30 hover:text-white/60'}`}>
    <div className={`p-3 rounded-2xl transition-all duration-500 flex items-center justify-center ${active ? 'bg-gradient-to-br from-cyan-500 to-blue-600 shadow-[0_10px_20px_rgba(6,182,212,0.3)] text-black scale-110' : ''}`}>{icon}</div>
...
const AdminNavBtn: React.FC<{ active: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 font-bold ${active ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/30' : 'text-white/40 hover:bg-white/10 hover:text-white'}`}>
