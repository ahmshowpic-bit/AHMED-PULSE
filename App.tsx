...
const App: React.FC = () => {
  ...
  const [isExtrasOpen, setIsExtrasOpen] = useState(false); // القائمة السفلية
  ...
  // Firebase Listeners - (تمت مراجعتها للتأكد من جلب كل البيانات)
  useEffect(() => {
  ...
  // --- الحل الجذري للتنقل ---
  // هذه الدالة ستضمن أن المشغل والقوائم تغلق عند الانتقال لأي تبويب
  const navigateTo = (tab: TabId) => {
    setActiveTab(tab);
    setIsPlayerExpanded(false); // إغلاق المشغل الكبير
    setIsExtrasOpen(false);     // إغلاق قائمة المزيد
  };
  ...
  return (
    <div className="relative h-screen w-full flex overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <section className={`${activeTab === 'home' ? 'block' : 'hidden'} animate-fade-in`}>
            <div className="min-h-[40vh] flex flex-col items-center justify-center text-center mt-8 md:mt-12 mb-20">
              {/* تعديل النص: break-words whitespace-normal */}
              <h2 className="text-4xl md:text-8xl font-black text-white mb-8 drop-shadow-2xl leading-tight px-4 break-words whitespace-normal max-w-full">
                {settings.welcome}
              </h2>
              {heroSong && (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 glass border-t border-white/10 z-[150] flex items-center justify-around px-4 bg-black/80 backdrop-blur-3xl">
        ...
        <div className="relative">
           <button 
             onClick={() => { setIsExtrasOpen(!isExtrasOpen); setIsPlayerExpanded(false); }}
      ...
      {/* Extras Menu (Fixed Z-Index) */}
      <div className={`md:hidden fixed inset-x-0 bottom-24 z-[200] transition-all duration-500 ease-in-out transform ${isExtrasOpen ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 pointer-events-none'}`}>
